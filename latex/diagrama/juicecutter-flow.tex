...
\node[act, below=0.5cm of s]      (init)   {Fruits start flying up from the bottom};
\node[act, below=0.5cm of init]   (play)   {The player swipes to cut the fruits the recipe asks for, adding to the score};
\node[dec, below=0.65cm of play]  (lost)   {Cut a wrong fruit with no lives left?};
\node[dec, below=0.7cm of lost]   (clear)  {Is the recipe complete?};
